...
from downloadHandler import handle_image_downloads, delete_images  # Ensure this import is correct
...
def process_image_batch(image_paths, folder, batch_num, is_test=False):
    ...
    # Remove the downloaded images to save space
    delete_images(image_paths, folder)
...
def main():
    ...
    train_image_folder = 'downloaded_images/train/'
    test_image_folder = 'downloaded_images/test/'
    ...
    train_image_paths = [img_file for img_file in train_data['image_link']]
    
    # Download, process, and delete images in batches
    for i in range(0, len(train_image_paths), batch_size):
        batch_paths = train_image_paths[i:i + batch_size]
        handle_image_downloads(pd.DataFrame({'image_link': batch_paths}), None, train_image_folder, None, delete_after_processing=False)
    ...
    test_image_paths = [img_file for img_file in test_data['image_link']]
    
    # Download, process, and delete images in batches
    for i in range(0, len(test_image_paths), batch_size):
        batch_paths = test_image_paths[i:i + batch_size]
        handle_image_downloads(None, pd.DataFrame({'image_link': batch_paths}), None, test_image_folder, delete_after_processing=False)
